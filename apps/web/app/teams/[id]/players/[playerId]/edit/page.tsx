...
export default function EditPlayerPage({ params }: { params: Promise<{ id: string, playerId: string }> }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('players').update({
      name, jersey_number: jerseyNumber ? parseInt(jerseyNumber) : null,
      position, gender, height,
      experience_level: experienceLevel, dominant_hand: dominantHand
    }).eq('id', playerId)
  ...
  return (
    <>
      ...
      <main className="max-w-xl mx-auto px-6 py-10">
        ...
        <h1 className="text-2xl font-bold text-slate-900 mt-4 mb-8">Edit Player</h1>
          ...
          <div className="p-6">
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-r from-cyan-500 to-cyan-600 text-white rounded-xl px-4 py-2.5 text-sm font-semibold hover:from-cyan-600 hover:to-cyan-700 disabled:opacity-50 transition-all shadow-sm shadow-cyan-200"
              >
              ...
              <button
                ...
                className="w-full border border-red-200 text-red-500 rounded-xl px-4 py-2.5 text-sm font-medium hover:bg-red-50 hover:border-red-300 transition-all"
              >
                Remove Player
              </button>
            </form>
